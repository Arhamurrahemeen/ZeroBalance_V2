# Phase 6 — Rahbar (Qdrant RAG, Urdu Q&A)
...
```
/backend/app
  rahbar.py             # corpus load, ensure_indexed (idempotent), retrieve, ask (Groq)
  rahbar_corpus.json    # 16 synthetic Urdu snippets (clearly marked مصنوعی ڈیمو)
  api.py                # + POST /rahbar/ask, GET /rahbar/queries
/backend/tests
  test_rahbar.py        # retrieval top-3 accuracy on the 10 queries; offline ask (fake Groq)
/docs
  rahbar_queries.md     # the 10 pre-tested queries + expected snippet
```
...
2. rahbar.py: lazy singleton embedder; ensure_indexed recreates collection iff point count differs; retrieve top-3; ask = retrieval + Groq (answer only from context, simple Urdu, cite source, say "don't know" if absent).
3. Routes: POST /api/v1/rahbar/ask {question} → {answer_ur, sources}; GET /api/v1/rahbar/queries → the 10 demo queries (for UI dropdown). Nothing else.
...
# live: POST /api/v1/rahbar/ask {"question": "دن کے اختتام پر کیش کیسے ملایا جائے؟"}
```

## What to expect
- Rebuild downloads the embedding model once (~120MB layer).
- pytest: 25 prior + rahbar tests green; 10/10 queries hit expected snippet in top-3.
- Live ask returns a grounded Urdu answer citing a corpus source.

## Achieved
- app/rahbar_corpus.json: 16 synthetic Urdu snippets, every source marked مصنوعی ڈیمو (test-enforced).
- app/rahbar.py: lazy multilingual embedder (fastembed extra, model baked in image), corpus-sha fingerprint in payloads so content edits trigger reindex, retrieve top-3, ask() grounded Groq answers (Urdu, cite ماخذ, refuse outside context).
- Routes: POST /rahbar/ask (503 no key, 502 upstream), GET /rahbar/queries (the 10 demo queries). No corpus management anywhere.
