...
2. **Groq explains, never decides.** Groq (free-tier model) receives the engine's already-ranked picks and produces Urdu explanations. If a change routes any decision, score, ranking, or filtering through Groq, it is wrong — stop and flag.
...
5. **On-prem posture.** No cloud services beyond the Groq API call. No customer data in logs sent anywhere external.
...
- Before starting a phase, create `/phases/phase_<n>.md` with: Phase Goal, Project Structure, Steps, Commands to run the phase, What to Expect (expected result of each command, so it can be diffed against reality). Minimal text.
